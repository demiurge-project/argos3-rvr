...
namespace argos
{
    const Real CCI_RVRWheelsActuator::MIN_VELOCITY_CM_SEC = -155.5; // 1.555 m/s is the actual value the RVR can support
    const Real CCI_RVRWheelsActuator::MAX_VELOCITY_CM_SEC = 155.5;
    ...
    int LuaSetLinearVelocity(lua_State *pt_lua_state)
    {
        ...
        /* Perform action */
        CLuaUtility::GetDeviceInstance<CCI_RVRWheelsActuator>(pt_lua_state, "wheels")->SetLinearVelocity(lua_tonumber(pt_lua_state, 1), lua_tonumber(pt_lua_state, 2));
        return 0;
    }
#endif
